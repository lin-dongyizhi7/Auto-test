        ...
        try:
            ...
            for app in desktop:
                if app.name == self.app.name:
                    # 找到应用的主窗口
                    for window in app:
                        if window.role == pyatspi.ROLE_WINDOW:
                            # 获取窗口位置和大小
                            x, y = window.queryComponent().getPosition(pyatspi.COORD_TYPE_SCREEN)
                            width, height = window.queryComponent().getSize()
    ...
    def _get_element(self, element_path: str, role_name_list: Optional[List[Optional[str]]] = None) -> Dict:
        ...
        try:
            ...
            for i, part in enumerate(remaining_path_parts):
                ...
                for child in current_element:
                    if child.name == part:
                        if current_role:
                            # 如果指定了角色，需要检查角色是否匹配
                            if hasattr(child, 'role') and child.role == current_role:
                                found_element = child
                                break
                        else:
                            # 没有指定角色，直接匹配名称
            ...
            try:
                ...
                x, y = component.getPosition(pyatspi.COORD_TYPE_SCREEN)
            ...
            store_data = {
                ...
                "role_name": current_element.roleName if hasattr(current_element, 'roleName') else str(current_element.role),
            ...
            if app_name:
                ...
                for app in desktop:
                    if app.name == app_name:
                        ...
                        break
                if not self.app:
                    print(f"未找到应用: {app_name}，将监控所有应用")
                    self.app = desktop
